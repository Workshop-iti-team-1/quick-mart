...
final class ProductDetailsViewModel: ObservableObject {
    ...
    @Published private(set) var isRefreshingInBackground: Bool = false
    ...
    init(
        ...
        preloadedProduct: ProductDetails? = nil,
    ...
    func loadProduct() {
        let hasPreloadedData = productDetails != nil
        if hasPreloadedData {
        ...
        Task {
            do {
                let details = try await getProductDetailsUseCase.execute(
                    id: productId)
                self.productDetails = details
                favouriteViewModel.syncIfFavorite(details)

                if let colorOption = details.options.first(where: {
                    $0.name.lowercased() == "color"
                }) {
                    self.selectedColor = colorOption.values.first
                }
                if let sizeOption = details.options.first(where: {
    ...
    private var selectedVariantId: String? {
        ...
        return productDetails.variants.first { variant in
            let matchesColor =
                selectedColor == nil
                || variant.selectedOptions.contains {
                    $0.name.lowercased() == "color" && $0.value == selectedColor
                }
            let matchesSize =
                selectedSize == nil
                || variant.selectedOptions.contains {
                    $0.name.lowercased() == "size" && $0.value == selectedSize
                }
    ...
    func addToCart(buyNow: Bool = false) async {
        ...
        if let variant = productDetails?.variants.first(where: {
            $0.id == variantId
        }) {
